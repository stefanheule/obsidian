...
/**
 * Update procedure for anything that changes with time
 */
static void time_update_proc(Layer * layer, GContext * ctx) {
    ...
    strftime(buffer_day, sizeof(buffer_day), "%a", t);
    text_layer_set_text(label_dayofweek, buffer_day);
    strftime(buffer_dayofweek, sizeof(buffer_dayofweek), "%d", t);
    ...
    // hour hand
    GPoint hour_hand = get_radial_point(radius * 0.7, ((t->tm_hour % 12) * 6) + (t->tm_min / 10), 12 * 6);
    graphics_context_set_stroke_width(ctx, 5);
    graphics_context_set_stroke_color(ctx, GColorBlack);
...
static void window_load(Window * window) {
    ...
    label_dayofweek = text_layer_create(GRect(46, 114, 27, 20));
    text_layer_set_text(label_dayofweek, buffer_day);
    text_layer_set_background_color(label_dayofweek, GColorBlack);
    text_layer_set_text_color(label_dayofweek, GColorWhite);
    text_layer_set_font(label_dayofweek, fonts_get_system_font(FONT_KEY_GOTHIC_18));
    layer_add_child(layer_time, text_layer_get_layer(label_dayofweek));

    // create day text layer
    label_day = text_layer_create(GRect(73, 114, 18, 20));
    text_layer_set_text(label_day, buffer_dayofweek);
    text_layer_set_background_color(label_day, GColorBlack);
    text_layer_set_text_color(label_day, GColorWhite);
    text_layer_set_font(label_day, fonts_get_system_font(FONT_KEY_GOTHIC_18_BOLD));
    layer_add_child(layer_time, text_layer_get_layer(label_day));
}
